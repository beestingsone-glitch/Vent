-- ==========================================================
-- Vent Database Schema & Row-Level Security (RLS)
-- "Let it out. Let it vanish."
...
CREATE TABLE users (
    ...
    avatar_url TEXT,
    bio TEXT DEFAULT 'Vent encrypted user.',
...
CREATE TABLE conversations (
    ...
    name TEXT, -- Applicable for group chats (e.g. 💨 Main Vent Room)
    avatar_url TEXT,
...
-- -------------------------------------------------------------
-- EPHEMERAL AUTO-DELETION CLEANUP (3 Days Main Chat / 24 Hours Direct Chat)
-- -------------------------------------------------------------
CREATE OR REPLACE FUNCTION purge_expired_vent_messages()
RETURNS void AS $$
BEGIN
    -- Delete messages exceeding TTL
    -- Main/Group chat: 3 days (72 hours)
    -- Private direct chat: 24 hours
...
-- SELECT cron.schedule('purge-ephemeral-vent', '0 * * * *', 'SELECT purge_expired_vent_messages()');
